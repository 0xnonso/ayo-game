...
library Ayo2x6Engine {
    ...
    function searchMoves(uint256 board, uint256 depth) internal pure returns (uint256) {
        ...
        for (; moves != 0x0; moves >>= 0x06) {
            ...
            currentScore = evaluateBoard(_board) + minimax(_board, alpha, beta, depth - 0x01, maximizing);
    ...
    function minimax(uint256 board, int256 alpha, int256 beta, uint256 depth, bool maximizing)
        internal
        pure
        returns (int256)
    {
        ...
        if (maximizing) {
            ...
            uint256 moves = Ayo.generateMoves(board);
            for (; moves != 0x0; moves >>= 0x06) {
                int256 score = minimax(Ayo.applyMove(board, moves & 0x3F), alpha, beta, depth - 0x01, false);
            ...
            uint256 moves = Ayo.generateMoves(board);
            for (; moves != 0x0; moves >>= 0x06) {
                int256 score = minimax(Ayo.applyMove(board, moves & 0x3F), alpha, beta, depth - 0x01, true);
