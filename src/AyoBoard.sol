...
library AyoBoard {
    ...
    function generateSVGBoard(uint256 board, uint256 newBoard, uint256 move)
        internal
        pure
        returns (string memory svg)
    {
        ...
        svg = string(
            abi.encodePacked(
                ...
                generateSVGScoreBoard(
                    Ayo.getP0PitsCaptured(board) * 0x04, Ayo.getP0PitsCaptured(newBoard) * 0x04, boardWidth, true
                )
            )
        );
        for (uint256 i = 0; i < boardWidth; i++) {
            uint256 boardPitSeedsNormalizedUpper = Ayo.getPitSeedsNormalised(totalBoardPits - i, board);
            uint256 boardPitSeedsNormalizedLower = Ayo.getPitSeedsNormalised(i, board);
            svg = string(
                abi.encodePacked(
                    svg,
                    generateSVGPitUpper(
                        ...
                        totalBoardPits,
                        totalBoardPits - i,
                        boardPitSeedsNormalizedUpper,
                        Ayo.getPitSeedsNormalised(totalBoardPits - i, newBoard),
                        boardPitSeedsNormalizedUpper != 0x01
                    ),
                    generateSVGPitLower(
                        move,
                        i,
                        boardPitSeedsNormalizedLower,
                        Ayo.getPitSeedsNormalised(i, newBoard),
                        boardPitSeedsNormalizedLower != 0x0
                    )
                )
            );
        }
        svg = string(
            abi.encodePacked(
                svg,
                generateSVGScoreBoard(
                    Ayo.getP1PitsCaptured(board) * 0x04, Ayo.getP1PitsCaptured(newBoard) * 0x04, boardWidth, false
                ),
                "</svg>"
            )
        );
    ...
    function generateSVGScoreBoard(uint256 oldPitCaptured, uint256 newPitCaptured, uint256 boardWidth, bool p0Board)
        internal
        pure
        returns (string memory svg)
    {
        ...
        for (uint256 i = 0; i < pitSeedMin; i++) {
            ...
            svg = string(
                abi.encodePacked(
                    svg, '<use href="#seed" class="seed" x="', String.toString(x), '" y="', String.toString(y), '"/>'
                )
            );
        }
